...
ANIMATION_SRC = "/animation_reflex.mp4"
...
def _discover_sample_pairs(max_cases: int = 7) -> dict[str, tuple[str, str]]:
    """Build dropdown options from complete left-right pairs.

    Preference order:
    1) assets/sample_cases (deploy-friendly static folder)
    2) preprocessed_images (local development fallback)
    """
    candidate_roots = [Path("assets/sample_cases"), Path("preprocessed_images")]
    ...
    for root in candidate_roots:
        ...
        for left_path in root.glob("*_left.*"):
            left_map[left_path.stem.removesuffix("_left")] = left_path
        for right_path in root.glob("*_right.*"):
        ...
        for case_id in common_ids[:max_cases]:
            lookup[f"Case {case_id}"] = (
                str(left_map[case_id]),
                str(right_map[case_id]),
            )
        return lookup
...
def left_sidebar() -> rx.Component:
    ...
    return rx.vstack(
        ...
        info_card(
            "Data Flow",
            rx.text("Input Images + Age -> Feature Processing -> Risk Prediction + Explanations", size="2"),
        ),
        info_card(
            "Tech Stack",
            rx.vstack(
                rx.text("Frontend: Reflex (Radix UI + Recharts)", size="2"),
                rx.text("API: FastAPI + Uvicorn + Pydantic + python-multipart", size="2"),
                rx.text("ML Core: PyTorch + Torchvision (EfficientNet-B4)", size="2"),
                rx.text("Image Pipeline: Albumentations + OpenCV + Pillow", size="2"),
                rx.text("Data/Metrics: NumPy + Pandas + scikit-learn + Matplotlib", size="2"),
                rx.text("Client/Networking: HTTPX", size="2"),
                spacing="1",
                align="start",
            ),
        ),
        ...
        info_card("Credits", rx.text("Made by Ayush Saini", size="2", weight="medium")),
        spacing="4",
        width="100%",
